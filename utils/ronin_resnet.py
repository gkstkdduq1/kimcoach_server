...
def run_test(network, data_loader, device, eval_mode=True):
    ...
    for bid, (feat, targ, _, _) in enumerate(data_loader):
        ...
        # make_dot(network(feat), params=dict(list(network.named_parameters()))).render("ronin_resnet", format="png")
...
def recon_traj_with_preds(dataset, preds, seq_id=0, **kwargs):
    ...
    # pos = interp1d(ts_ext[:pos.shape[0]], pos, axis=0)(ts_ext[:pos.shape[0]])
    return pos
...
def test_sequence(args):
    ...
    for data in test_data_list:
        ...
        seq_loader = DataLoader(seq_dataset, batch_size=128, shuffle=False, drop_last=True)
        ...
        # seq_dataset.gt_pos[0][:, 0],seq_dataset.gt_pos[0][:, 1]  = p(seq_dataset.gt_pos[0][:, 0], seq_dataset.gt_pos[0][:, 1])
        pos_pred = recon_traj_with_preds(seq_dataset, preds)[:, :2]
        pos_gt = seq_dataset.gt_pos[0][:, :2]
        # pos_pred[:,1], pos_pred[:,0] = p(pos_pred[:,0], pos_pred[:,1], inverse = True)
        # pos_gt[:,1], pos_gt[:,0] = p(pos_gt[:,0], pos_gt[:,1], inverse=True)
        ...
        if args.out_dir is not None and osp.isdir(args.out_dir):
            ...
            np.savetxt(osp.join(args.out_dir, data + "location_gt.csv"), pos_gt[:, :2], delimiter=",")
            np.savetxt(osp.join(args.out_dir,data+"location_pred.csv"), pos_pred[:, :2], delimiter=",")
...
if __name__ == '__main__':
    ...
    args.mode = 'test'
    args.test_list = '../lists/list_test_unseen.txt'
    args.root_dir = '../data'
    args.out_dir = '../output'
    args.model_path = '../saved_model/checkpoint_gsn_latest.pt'
